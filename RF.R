...
set.seed(42)
index <- createDataPartition(tmp_df$Stem15, p = 0.8, list = FALSE)
train_data <- tmp_df[index, ]
test_data <- tmp_df[-index, ]
...
ptm <- proc.time()
set.seed(1234)
model.rf <- train(Stem15 ~ .,
                  data = train_data,
                  method = "rf",
                  ntree = 1000, # How many trees to grow in total?
...
#------------- R square 
RMSE <- sqrt(sum((pred - test_data$Stem15)^2)/length(pred))
print(RMSE)
print(RMSE/mean(test_data$Stem15)) 
# 
postResample(pred = pred, obs = test_data$Stem15)



actual<-test_data$Stem15
...
ggplot(result,aes(x=actual,y=predicted,color=predicted-actual),alpha=0.7)+
  ...
  ggtitle('Predicted RF Model')+
  labs(x="Actual",y="Predicted")


# caret has a function to calculate variable importance so that you can see 
# which variables were the most informative
# in making distinctions between classes.
# plot(varImp(model.rf, scale = FALSE))

ImpMeasure<-varImp(model.rf, scale = FALSE)
plot(ImpMeasure,top = 20 ,title="sd")
